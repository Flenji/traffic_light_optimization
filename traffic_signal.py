...
EXTRACTED FROM: https://github.com/LucasAlegre/sumo-rl/blob/main/sumo_rl/environment/traffic_signal.py
MODIFIED BY: AAU CS (IT) 07-03"""
import os
import sys
...
class TrafficSignal:
    ...
    def __init__(
        ...
        self.links = list(dict.fromkeys(self.sumo.trafficlight.getControlledLinks(self.id)))
        self.incoming_edges = list(set([traci.lane.getEdgeID(link[0]) for link in self.links]))
        self.outgoing_edges = list(set([traci.lane.getEdgeID(link[1]) for link in self.links]))
        ###
    ...
    def _incoming_edge_congestion_reward(self):
        ...
        congestions = self.get_incoming_edges_density()

        return -sum(congestion^2 for congestion in congestions)
        

    def _long_waits_penalize(self):
        ...
        for edgeID in self.incoming_edges:
            max_time_edge = -1
            vehicles = self.sumo.edge.getLastStepVehicleIDs(edgeID)
            for vehicleID in vehicles:
                wait = self.sumo.vehicle.getWaitingTime(vehicleID)
                if wait > max_time_edge:
                    max_time_edge = wait
            sum_max_waits += max_time_edge

        return -min(4*95, sum_max_waits/(4*95))
    
    def _combined_reward1(self):
        """First reward function defined combining several factors."""
        return 0.70* self._incoming_edge_congestion_reward() + 0.30* self._long_waits_penalize()


    ### ADITIONAL FUNCTIONS
    
    def get_incoming_edges_density(self) -> List[float]:
        ...
        """
        edges_density = [self.sumo.edge.getLastStepOccupancy(edgeID)/100 for edgeID in self.incoming_edges]
        return [min(1, density) for density in edges_density]

    def get_edges_queue(self) -> List[int]:
        """Returns the number of queued vehicles of the vehicles in the incoming edges of the intersection.
        """
        edges_queued = [self.sumo.edge.getLastStepHaltingNumber(edgeID) for edgeID in self.incoming_edges]
        return edges_queued
    ...
    reward_fns = {
        ...
        "incoming-edge-congestion": _incoming_edge_congestion_reward,
        "long-waits": _long_waits_penalize,
        "combined1": _combined_reward1,
    }
